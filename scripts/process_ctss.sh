...
t) TPM=${OPTARG};;
o) OUTDIR=${OPTARG};;
*) echo "Usage is : $0 -t <tpm_threshold> -o <output_dir> <ctss_file1> <ctss_file2>" >&2
    exit 1;;
esac
done
...
# "Filter ctss < $TPM TPM"
 ...
 awk -v ctssall="$counts" -v TPM="$TPM" '{if (($5/ctssall)*1000000>=TPM) print}' ctssAll > ctssAll_${TPM}_tpm_new
